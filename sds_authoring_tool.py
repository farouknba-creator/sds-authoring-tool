...
def extract_sections_from_pdf(pdf_path):
    ...
    # Split into sections using common GHS headings (case‑insensitive)
    ...
    if len(splits) <= 1:
        # No recognised headings – return the whole text as one block
        return [text.strip()]

    sections = []
    # First part (before first heading) is ignored (supplier block)
